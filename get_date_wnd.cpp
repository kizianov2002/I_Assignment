    ...
    for (int i=QDate::currentDate().year(); i>=date_min.year(); i--){
        ...
        if (date_min.year()==date_max.year()) {
            if ( (date_min.month()<=12 && date_max.month()>=12 ) )
                ui->combo_month->addItem(QString::number(i) + ".12 - декабрь", QDate(i,12,1));
            if ( (date_min.month()<=11 && date_max.month()>=11 ) )
                ui->combo_month->addItem(QString::number(i) + ".11 - ноябрь", QDate(i,11,1));
            if ( (date_min.month()<=10 && date_max.month()>=10 ) )
                ui->combo_month->addItem(QString::number(i) + ".10 - октябрь", QDate(i,10,1));
            if ( (date_min.month()<= 9 && date_max.month()>= 9 ) )
                ui->combo_month->addItem(QString::number(i) + ".09 - сентябрь", QDate(i,9,1));
            if ( (date_min.month()<= 8 && date_max.month()>= 8 ) )
                ui->combo_month->addItem(QString::number(i) + ".08 - август", QDate(i,8,1));
            if ( (date_min.month()<= 7 && date_max.month()>= 7 ) )
                ui->combo_month->addItem(QString::number(i) + ".07 - июль", QDate(i,7,1));
            if ( (date_min.month()<= 6 && date_max.month()>= 6 ) )
                ui->combo_month->addItem(QString::number(i) + ".06 - июнь", QDate(i,6,1));
            if ( (date_min.month()<= 5 && date_max.month()>= 5 ) )
                ui->combo_month->addItem(QString::number(i) + ".05 - май", QDate(i,5,1));
            if ( (date_min.month()<= 4 && date_max.month()>= 4 ) )
                ui->combo_month->addItem(QString::number(i) + ".04 - апрель", QDate(i,4,1));
            if ( (date_min.month()<= 3 && date_max.month()>= 3 ) )
                ui->combo_month->addItem(QString::number(i) + ".03 - март", QDate(i,3,1));
            if ( (date_min.month()<= 2 && date_max.month()>= 2 ) )
                ui->combo_month->addItem(QString::number(i) + ".02 - февраль", QDate(i,2,1));
            if ( (date_min.month()<= 1 && date_max.month()>= 1 ) )
                ui->combo_month->addItem(QString::number(i) + ".01 - январь", QDate(i,1,1));
...
void get_date_wnd::on_bn_save_clicked() {
    if (ui->rb_date->isChecked()) {
        ...
        date2 = date2.addYears(1).addDays(-1);
    } else {
