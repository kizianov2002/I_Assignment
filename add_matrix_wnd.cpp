...
#include "ui_add_matrix_wnd.h"

add_matrix_wnd::add_matrix_wnd(int &layer_type, int &assig_type, int &id_layer_old, QDate &layer_date_old, int &id_layer_new, QDate &layer_date_new, QSqlDatabase &db, s_data_app &data_app, QSettings &settings, QWidget *parent) :
...
add_matrix_wnd::~add_matrix_wnd() {
    delete ui;
}


void add_matrix_wnd::refresh_combo_layers() {
...
void add_matrix_wnd::on_combo_layer_new_currentIndexChanged(int index) {
    layer_type = (ui->rb_inko->isChecked() ? 1 : 2);
...
void add_matrix_wnd::on_combo_layer_old_currentIndexChanged(int index) {
    layer_type = (ui->rb_inko->isChecked() ? 1 : 2);
...
void add_matrix_wnd::on_rb_inko_clicked(bool checked) {
    refresh_combo_layers();
}
void add_matrix_wnd::on_rb_tfoms_clicked(bool checked) {
    refresh_combo_layers();
}


void add_matrix_wnd::on_bn_process_clicked() {
    if (id_layer_old<1) {
...
void add_matrix_wnd::on_bn_cansel_clicked() {
    reject();
}
