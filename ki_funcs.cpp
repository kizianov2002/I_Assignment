...
QString show_rubles(double num) {
    ...
    while (num_1>0) {
        int n = num_1 % 10;
        str = QString::number(n) + str;
        num_1 /= 10;
    }
    if (str.isEmpty())  str = "0";

    // разберём копейки
    int num_2 = (num - long(num) - 0.001) * 100;

    str += "," + QString("00" + QString::number(num_2)).right(2);
...
bool table2csv(QSqlDatabase &db, QString sql, QSqlQueryModel &model, QTableView *table, bool f_open, QString path, QWidget *parent, QStringList names_old, QStringList names_new) {
    ...
    while (query_export->next()) {
        for (int j=0; j<cnt_head; j++) {
            if (table->columnWidth(j)>5) {
                ...
                } else if ( query_export->value(j).type()==QMetaType::Float
                            || query_export->value(j).type()==QMetaType::Double ) {
                    value = query_export->value(j).toString().trimmed();
                    value = value.replace(".", ",");
                } else {
...
bool sql2csv(QSqlDatabase &db, QString sql, bool f_open, QString fname_s, QWidget *parent, QStringList names_old, QStringList names_new) {
    if (QFile::exists(fname_s)) {
        if (!QFile::remove(fname_s)) {
            QMessageBox::warning(parent,
                                 "Ошибка при удалении старого файла",
    ...
    if (!file.open(QIODevice::WriteOnly)) {
        ...
        if (QFile::exists(fname_s)) {
            while (!QFile::remove(fname_s)) {
                if (QMessageBox::warning(parent,
                                     "Ошибка при удалении старого файла",
                                     "При удалении старого файла произошла непредвиденная ошибка.\n\n"
                                     "Повторить попытку?", QMessageBox::Retry|QMessageBox::Abort, QMessageBox::Abort)==QMessageBox::Abort)
                    return false;
            }
        }
    }
...
bool read_csvData(QString &text_s, QVector<QStringList> &data_lists, int &data_lists_cnt, QWidget *parent) {
    ...
    for (int i=0; i<text_list.size(); i++) {
        ...
        // выжеденные поля разложим по спискам
        while (pos<len && pos!=-1) {
        ...
        n_value++;
        // остальные списки добъём пустыми строками
        while (n_value<data_lists.count()) {
            data_lists[n_value].append("");
            n_value++;
        }
        line_counter++;
    }
